...
class TdpProfileImport implements ToModel, WithHeadingRow, WithBatchInserts, SkipsEmptyRows
{
    ...
    public function model(array $row)
    {
        ...
        $seq = isset($row['seq']) ? trim($row['seq']) : null;
        
        // ---------------------------------------------------------
        // 2. IDENTIFY SCHOLAR (AND HANDLE EMAIL CONFLICTS)
        // ---------------------------------------------------------
        ...
        // Sanitize Email
        $rawEmail = trim($row['email_add'] ?? '');
        $email = $this->isValidEmail($rawEmail) ? $rawEmail : null;

        // A. FIND Existing Scholar by Name
        ...
        // C. SAVE Scholar (Create or Update)
        if (!$scholar) {
            $scholar = new Scholar();
        ...
        if ($rawCourse) {
            ...
            // B. Find Course ID (Using Smart Matcher)
        ...
        // ---------------------------------------------------------
        // 9. ACADEMIC RECORD & VALIDATION LOGIC
        // ---------------------------------------------------------
        $validatorName = $row['validated_by'] ?? $row['verified_by'] ?? null;
        ...
        // 10. BILLING RECORD
        // ---------------------------------------------------------
        $validatorId = $this->uploaderId; // Default
        ...
        BillingRecord::updateOrCreate(
            ...
            [
                ...
                'status' => (!empty($validatorName) ? 'Validated' : 'Processed'), 
            ]
        );
    ...
    private function processAddress($scholar, $row)
    {
       ...
       $brgyId = null;
        if ($brgyName && $cityId) {
             // We try to find the row first
             $brgy = Barangay::where('barangay', $brgyName)
                             ->where('cityID', $cityId)
                             ->first();
             
             if ($brgy) {
                 // Check if 'barangayID' attribute exists, otherwise fall back to 'id'
                 // This fixes the "Unknown column 'id'" error if your table uses 'barangayID' as PK
                 $brgyId = $brgy->barangayID ?? $brgy->id;
             }
        }
