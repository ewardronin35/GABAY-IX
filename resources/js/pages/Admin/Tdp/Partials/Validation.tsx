...
export default function Validation({ auth, scholars, filters, academicYears, batches, courses }: ValidationPageProps) {
    ...
    const updateParams = (key: string, value: string | null) => {
        router.get(
            route("superadmin.tdp.validation.index"), 
    ...
    const onMainTabChange = (value: string) => {
        ...
        router.visit(route("superadmin.tdp.index", { tab: value }));
    };
...
const handlePrintBatch = () => {
        ...
        // 3. Generate URL
        const url = route('superadmin.tdp.generate-batch-noa', {
                                ...
                                <div className="flex flex-wrap gap-2 items-center bg-muted/30 p-2 rounded-md border border-dashed justify-between">
                                    <div className="flex flex-wrap gap-2 items-center">
                                        ...
                                        {(filters.academic_year || filters.course || filters.batch) && (
                                            <Button variant="ghost" size="sm" onClick={() => router.get(route('superadmin.tdp.validation.index'))} className="h-8 text-destructive">Reset</Button>
                                        )}
                                    </div>
                                                ...
                                                <TableRow key={item.id}>
                                                    ...
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-2">
                                                            ...
                                                            {item.payment_status === 'Validated' && (
                                                                <a 
                                                                    href={route('superadmin.tdp.generate-noa', item.id)} 
                                                                    target="_blank" 
                                                                    rel="noreferrer"
                                                                >
