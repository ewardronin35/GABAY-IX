...
export function ValidationModal({ isOpen, onClose, enrollmentId }: ValidationModalProps) {
    ...
    const fetchChecklist = async () => {
        setIsLoading(true);
        try {
            const response = await axios.get(route('superadmin.tdp.validation.checklist', enrollmentId));
    ...
    const handleApprove = () => {
        ...
        router.post(route('superadmin.tdp.validation.approve', enrollmentId), {}, {
            onSuccess: () => {
                                        ...
                                        {activeUploadId === item.id && (
                                            <div className="mt-3 animate-in fade-in zoom-in-95">
                                                ...
                                                <FilePond
                                                    ...
                                                    server={{
                                                        url: route('superadmin.tdp.validation.upload', enrollmentId),
                                                        process: {
                                                            headers: {
