...
import { MasterlistEditModal } from "./MasterlistEditModal"; // Import the new modal
...
export const columns: ColumnDef<TdpRowData>[] = [
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <Link 
                    href={route("superadmin.tdp.scholar.show", scholarId)} 
    ...
    },
    {
        id: "program", 
        header: "Program",
        accessorFn: (row) => row.enrollment?.program?.program_name || 'TDP',
        cell: ({ row }) => {
        ...
        cell: ({ row }) => {
            return (
                <div className="flex items-center gap-1">
                    {/* The NEW Masterlist Edit Modal */}
                    <MasterlistEditModal record={row.original} />
                    
                    {/* View Icon */}
                    {row.original.enrollment?.scholar?.id && (
                        <Link href={route("superadmin.tdp.scholar.show", row.original.enrollment.scholar.id)}>
