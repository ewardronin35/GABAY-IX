...
// --- ✨ NEW MAIN COMPONENT ---
export function AccountingApprovalSheet({ request }: { request: FullFinancialRequest }) {
    ...
    const handleApprove = () => {
        ...
        router.post(route('accounting.approve', request.id), {}, {
            preserveScroll: true,
    ...
    const handleConfirmReject = () => {
        if (!remarks) return;
        setIsSubmitting(true);
       router.post(route('accounting.reject', request.id), { remarks }, {
            preserveScroll: true,
            onFinish: () => {
...
const isActionable = request.status === 'pending_accounting';
    return (
        <>
            ...
            {isActionable && (
                <Card className="my-4 bg-blue-50 border-blue-200">
                    ...
                    <CardContent className="flex items-center gap-4">
                        <Button className="flex-1" size="lg" onClick={handleApprove} disabled={isSubmitting}>
                            <Check className="mr-2 h-4 w-4" /> Approve
                        </Button>
                        <Button className="flex-1" size="lg" variant="destructive" onClick={() => setIsRejectModalOpen(true)} disabled={isSubmitting}>
