...
const mapDataToGrid = (records: any[]) => {
    ...
    return records.map((record) => {
        ...
        return {
            ...
            program_name: enrollment.program?.program_name || 'TES',
    ...
    return (
        <>
            <style>{`
                ...
                .dark .handsontable th, .dark .ht_clone_top th, .dark .ht_clone_left th, .dark .ht_clone_top_left_corner th {
                    background-color: #18181b !important; color: #a1a1aa !important;
                    border-color: #27272a !important; font-weight: 600;
                }
                .dark .handsontable tbody tr td {
                    background-color: #09090b !important; color: #f8fafc !important;
                    border-bottom: 1px solid #27272a !important; border-right: 1px solid #27272a !important;
                }
                ...
                .dark .htDropdownMenu table.htCore, .dark .htContextMenu table.htCore {
                    background-color: #18181b !important; border: none !important;
                }
                .dark .htItemWrapper:hover {
                    background-color: #27272a !important; color: #fff !important; cursor: pointer;
                }
...
// --- 3. MAIN COMPONENT ---
export function TesDatabaseGrid({
    databaseData,
    filters,
    academicYears,
    semesters,
    ...
    const columns = useMemo(() => [
        ...
        { data: 'student_id', title: 'Student ID' }, // New
        { data: 'eligibility', title: 'Eligibility' }, // New
    ...
    const handleSave = useCallback(() => {
        ...
        setIsSaving(true);
        router.post(route('admin.tes.bulkUpdate'), { enrollments: cleanedData }, {
            preserveScroll: true,
            onSuccess: () => { 
    ...
    const handleDelete = () => {
        ...
        setIsDeleting(true);
        router.post(route('admin.tes.bulk-destroy'), { ids: selectedIds }, {
            preserveScroll: true,
            onSuccess: () => { 
    ...
    const handleSheetChange = (value: string) => {
        ...
        router.get(route('admin.tes.index'), { ...filters, [filterKey]: value }, {
            preserveState: true, 
            preserveScroll: true, 
            only: ['database_tes', 'filters'] 
        });
    };
    ...
    const renderSheetTabs = () => {
        ...
        return (
            <div className="flex items-center space-x-1 overflow-x-auto bg-gray-100 p-2 rounded-b-lg border-t border-gray-200 mt-0 dark:bg-zinc-900 dark:border-zinc-800">
                <Button variant={!activeValue ? "secondary" : "ghost"} size="sm" className="h-8 text-xs dark:text-zinc-300" onClick={() => handleSheetChange('')}><LayoutList className="mr-2 h-3 w-3" /> All</Button>
                {options.map((opt) => (
                    <Button key={opt.value} variant={activeValue === opt.value ? "default" : "ghost"} size="sm" className={`h-8 text-xs min-w-[80px] ${activeValue === opt.value ? 'bg-zinc-700 text-white' : ''}`} onClick={() => handleSheetChange(opt.value)}>{opt.label}</Button>
                ))}
            </div>
        );
    };
        ...
        <Card className="flex flex-col h-auto min-h-[700px] dark:bg-zinc-950 dark:border-zinc-800 w-full max-w-[100vw] overflow-hidden">
            <CardHeader className="pb-2">
                <div className="flex justify-between items-center">
                    <div>
                        <CardTitle className="dark:text-white">TES Masterlist Database</CardTitle>
                        <CardDescription className="dark:text-zinc-400">Manage TES scholars, payments, and enrollment data.</CardDescription>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" size="sm" className="h-8 gap-2 dark:bg-zinc-900 dark:text-white dark:border-zinc-800">
                                    ...
                                    {sheetMode === 'batch' ? 'View: Batch' : sheetMode === 'academic_year' ? 'View: A.Y.' : sheetMode === 'semester' ? 'View: Sem' : 'Sheets Off'}
                                </Button>
                            </DropdownMenuTrigger>
                        ...
                        <Button variant="destructive" size="sm" onClick={handleDelete} disabled={isDeleting || selectedIds.length === 0}><Trash2 className="h-4 w-4" /></Button>
                        <Button size="sm" onClick={handleSave} disabled={isSaving} className="bg-green-600 hover:bg-green-700 text-white"><Upload className="h-4 w-4 mr-2" /> Save</Button>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 bg-gray-50 p-1 rounded border border-gray-300 mt-2 dark:bg-zinc-900 dark:border-zinc-800">
                    <div className="flex items-center text-muted-foreground px-2 border-r gap-1 min-w-[60px] justify-center bg-gray-100 h-full dark:bg-zinc-800 dark:border-zinc-800">
                        <Sigma className="h-4 w-4 text-gray-500 dark:text-zinc-400" /><span className="text-xs font-mono font-bold text-gray-700 dark:text-zinc-300">{activeCoords ? `${String.fromCharCode(65 + activeCoords.c)}${activeCoords.r + 1}` : ''}</span>
                    </div>
                    <Input className="h-8 border-0 shadow-none focus-visible:ring-0 bg-transparent flex-1 font-mono text-sm dark:text-white" placeholder="Enter value or formula..." value={activeCellVal} onChange={handleFormulaChange} />
                    <div className="relative w-64 hidden md:block"><Search className="absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input className="h-8 pl-8 text-xs dark:bg-zinc-800 dark:border-zinc-800 dark:text-white" placeholder="Search grid..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
                </div>
            ...
            <CardContent className="p-0 bg-white border-t border-b flex-1 dark:bg-zinc-950 dark:border-zinc-800 w-full max-w-[100vw] overflow-hidden">
                ...
                <div className="p-2 bg-gray-50 dark:bg-zinc-900"><PaginationLinks links={paginationLinks} /></div>
            </CardContent>
        </Card>
    );
}