...
import { columns } from "./TesHeiGridColumns"; 
...
import { HeiFileUpload } from './HeiFileUpload'; 
import { useMemo, useState, useEffect } from 'react';

type LocationItem = { id: number; name: string; region_id?: number; province_id?: number };

type TesShowHeiProps = PageProps & {
    hei: HEI;
    enrollments: any; // 'any' for safe pagination access
    documents: any[];
    filters?: { 
        search?: string;
...
export default function TesShowHei({ 
    auth, 
    hei, 
    enrollments, 
    filters = {}, 
    ...
    const { handleSearch } = useSearch(
        route('admin.tes.hei.show', hei.id),
        filters.search || "",
        "search"
    );
    ...
    const clearFilters = () => {
        setSearchTerm("");
        router.get(route('admin.tes.hei.show', hei.id));
    };
    ...
    return (
        <AuthenticatedLayout user={auth.user} page_title={`${hei.hei_name} - TES Grantees`}>
            ...
            <div className="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <Link href={route('admin.tes.index', { tab: 'hei' })}>
                    ...
                    <CardHeader>
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                ...
                                <CardDescription>
                                    All TES grantees enrolled at this institution.
                                </CardDescription>
                            </div>
