...
// --- ✨ FIX 1: Fill in all helper functions ---
...
const getAttachmentUrl = (path: string) => {
    // This assumes your public storage is linked and files are in 'storage/app/public'
    // If you are using the 'private' disk, you must create a dedicated download route.
...
function StepperStep({ 
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            <p className={cn(
                "text-xs mt-2 font-semibold text-center", // Added text-center
...
function RequestStepper({ request }: { request: FullFinancialRequest }) {
    ...
    return (
        <div className="flex items-start p-4 my-4 bg-muted/50 rounded-lg">
            ...
            <div className={cn(
                "flex-1 border-b-2 mt-5 mx-2",
                budgetStatus !== 'pending' ? "border-green-500" : "border-muted-foreground/30 border-dashed"
            )} />
            <StepperStep icon={<Landmark className="w-5 h-5" />} title="Budget" status={budgetStatus} />
            <div className={cn(
                "flex-1 border-b-2 mt-5 mx-2",
                accountingStatus !== 'pending' ? "border-green-500" : "border-muted-foreground/30 border-dashed"
            )} />
            <StepperStep icon={<Calculator className="w-5 h-5" />} title="Accounting" status={accountingStatus} />
            <div className={cn(
                "flex-1 border-b-2 mt-5 mx-2",
                cashierStatus !== 'pending' ? "border-green-500" : "border-muted-foreground/30 border-dashed"
            )} />
...
// --- MAIN VIEW REQUEST SHEET COMPONENT ---
export function ViewRequestSheet({ request }: { request: FullFinancialRequest }) {
    return (
        <>
