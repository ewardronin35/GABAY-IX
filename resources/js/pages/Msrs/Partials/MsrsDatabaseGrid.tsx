...
const mapDataToGrid = (records: any[]) => {
    ...
    const safeText = (val: any, fallbackKey = 'name') => {
        if (!val) return '';
        if (typeof val === 'string' || typeof val === 'number') return String(val);
        if (typeof val === 'object') return val[fallbackKey] || val.name || '';
        return '';
    };
    ...
    return records.map((record) => {
        ...
        return {
            ...
            // Financials
            grant_amount: record.grant_amount ? parseFloat(record.grant_amount) : 0,
            ...
            city_municipality: safeText(address.city) || address.town_city || '',
        };
    });
};
...
export function MsrsDatabaseGrid({ enrollments, filters, academicYears = [], semesters = [], heiList }: any) {
    ...
    // ✅ FIX: Removed 'readOnly: true' from all columns so you can edit them
    const columns = useMemo(() => [
        { data: 'seq', title: 'Seq', width: 60 },
        ...
        { data: 'hei_name', title: 'HEI', width: 200 },
        { data: 'course_name', title: 'Course', width: 150 },
        { data: 'year_level', title: 'Yr', width: 50 },
        ...
        { 
            ...
            width: 120,
            className: 'htRight text-blue-600 font-bold' 
        },
        { 
            data: 'validation_status', 
            title: 'Payment Status', 
            type: 'dropdown',
            source: ['Pending', 'Validated', 'Paid', 'Rejected'],
            width: 120
        },
        ...
        { data: 'city_municipality', title: 'City', width: 120 },
    ], []);
    ...
    const handleSave = () => {
        ...
        setIsSaving(true);
        router.post(route('admin.msrs.bulk-update'), { enrollments: tableData }, {
            onSuccess: () => {
            ...
            onError: () => {
                toast.error("Failed to save changes. Check inputs.");
                setIsSaving(false);
            }
        });
    };
    ...
    const handleSheetChange = (value: string) => {
        ...
        let filterKey = 'academic_year'; 
        if (sheetMode === 'semester') filterKey = 'semester';

        router.get(route('admin.msrs.index'), { 
            ...filters, 
            [filterKey]: value === 'all' ? null : value,
            page: 1,
            limit: 2000 // Ensure we keep loading a large dataset
            ...
            <CardHeader className="pb-2">
                ...
                <div className="mt-2">
                    <FilterBar 
                        ...
                        semesters={sheetMode === 'semester' ? [] : semesters} 
                        heiList={heiList} 
                        batches={[]} 
                        courses={[]}
                    />
